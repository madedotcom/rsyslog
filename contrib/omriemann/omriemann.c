...
static void closeRiemannClient(wrkrInstanceData_t *pWrkrData)
{
	if(pWrkrData->client != NULL) {
		riemann_client_disconnect(pWrkrData->client);
...
if((pvals = nvlstGetParams(lst, &actpblk, NULL)) == NULL)
	...
	for(i = 0 ; i < actpblk.nParams ; ++i) {
		...
		} else if(!strcmp(actpblk.descr[i].name, "ttl")) {
			pData->ttl = (uchar*)es_str2cstr(pvals[i].val.d.estr, NULL);
		}
	}
...
pData->propTTL = getPropertyDescriptor(pData->ttl);
